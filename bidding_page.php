				...
				<div class="container clearfix">
					...
					<div class="postcontent nobottommargin clearfix">
						...
						<form method = "post">
                            ...
                            <input name = "id" value =	<?php echo $item_id;?> type = "hidden"></input>
								<tbody>
									<tr>
										<?php
										// PHP code to display the item picture
							...
							<table style="display: inline-block" class="table">
								<tbody>
									...
									<tr>
										...
										<?php
											// PHP code to display curren titem price
											$sql = "SELECT * FROM `items` WHERE `itemid` = $item_id";
											$result = mysqli_query($conn, $sql);
											$resultCheck = mysqli_num_rows($result);
											if ($resultCheck > 0) {
									...
									<tr>
										...
										<td>
											<input type="submit" id="place_bid" style="width:100px; height:25px" name = "placebid"  value = "Place bid"
												<?php
													// TO enable/disable bidding button based on the bidding time
											...
											<input type="submit" id="pay" style="width:100px; height:25px" name = "pay"  value = "Pay"
												<?php
													// PHP code to enabel/disable pay button
							...
							<table class="table">
								<tbody>
									<tr>
										...
										<?php
											// PHP code to show the number of bids
										...
										<td>Bid Duration: </td>
										<?php
										// PHP code to display the bid duration
											$sql = "SELECT * FROM `items` WHERE `itemid` = $item_id";
											$result = mysqli_query($conn, $sql);
											$resultCheck = mysqli_num_rows($result);
											if ($resultCheck == 0) {
							...
							<table id="all_bids" class="table table-striped">
								...
								<?php
									...
									// Iteratively show all the previous bids
									else if ($resultCheck > 0) {
										date_default_timezone_set('America/New_York');
										echo "<tbody>";
										while ($row = mysqli_fetch_assoc($result)) {
						...
						<?php
							// PHP code for the from processing
							// Processing for place bid button
							if(isset($_POST["placebid"])) {
								...
								while ($row = mysqli_fetch_assoc($result)) {
									...
									{
										// Insert this new bid into the bidding table
										// Update the item table with this new bid price an dthe current user
										// Update the buyers information
										$t = date("Y-m-d H:i:s");
